...
const DATA = generateEmployees(60);

// Grouping, sorting and pagination sync to the URL. The grouped default is a
// single page holding every row (pageSize = DATA.length) — see setGrouped.
export const groupingPaginationSearch = createTableSearchConfig({
  defaults: {
    grouping: ["department"],
...
const routeApi = getRouteApi("/grouping-pagination");

export function GroupingPaginationExample() {
  const data = DATA;

  const { atoms, tableOptions } = useTableUrlSync({
    route: routeApi,
    scope: "examples/grouping-pagination",
    defaults: groupingPaginationSearch.defaults,
  });
  ...
  const isGrouped = useSelector(atoms.grouping).length > 0;
